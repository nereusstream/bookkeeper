...
2026-09-18收敛修订；规划基线`ffebee6f59a9c4113eabda39f5da41fe305616a2`。以下全部为**PLANNED / NOT EXECUTED**，没有新增源码修复、实现/故障测试或PASS receipt。本轮纠正Classic恢复基线，复用既有quorum/fencing/顺序write-back并补逐来源否定条件；在DATA准入前保留完成工作集；明确冻结source及成组compaction净收益；清理Arena重复生命周期记录和单ledger式FREE模板。此前多slot ACK gate、统一entry上限、共享文件恢复、双层磁盘预算及延期范围保持，reference codec oracle与历史证据不重绑。
...
| UW-3 最小真实写入路径 | 保留ByteBuf、固定shard/shared block、预分配及完整write/barrier→物理前缀→热定位→逐entry成功。DATA准入前保障共享提交buffer和定位/完成容量，输入不能挤占；索引异步接管不等flush，stall先背压新DATA。Bookie持有route/fence/grant/tombstone，Arena不重复写生命周期记录 | RFC-0003 §5/6.1/13/15、RFC-0005 §5.2/10.1、[Spike B B2/B9/B16/B18](../spikes/SPIKE-B-allocator-block.md)；入口近满暂停/恢复下游仍完成已接受请求 |
| UW-4 allocator恢复与复用 | 保留共享文件逻辑尾部、旧generation残留、I/O隔离及selector/pin规则。FREE以allocation/generation为主语，delete/compaction/orphan原因不替代整单元无live occupant/写I/O/pin条件；正常compaction只选冻结且写入已解析source，同source/generation一项任务，OPEN ledger无需close/fence，异常竞争仍条件化 | [RFC-0003 §5/6/10/12](../RFC-0003-segment-storage-allocator.md)、B2/B3/B4/B6/B9/B10/B16、C-TAIL/C-REUSE/C-MOVE |
...
| UW-6 最小集群可恢复性 | 实验3/3/2与3/3/3、Bookie故障域F=1；复用Classic write-set、fencing read、W-A+1聚合/顺序write-back，适配Profile身份/grant/unknown与durability。每个否定来源自身durably fenced且pre-cut写/定位已解析，重复/旧身份不计；durable close、普通删除及延期边界保持 | [RFC-0004 §7.3—7.5](../RFC-0004-range-recovery-delete.md)、RFC-0005 §6—8、A16/A26/B19、A-POINT/D-LOGICAL |
| UW-7 全量重建与总资源 | 保留关闭纯派生索引WAL、真实persisted coverage及完整重建；unknown不伪造absence。hot/async index/memtable/读buffer/pin与共享提交/完成工作集同一总账，交接计费，满载无循环资源依赖；大entry先取得工作集。保留SST/控制磁盘峰值和共享filesystem去重 | RFC-0003 §9/12/13/15、RFC-0005 §8/10.1、B2/B3/B9/B11/B12/B17/B18，dedicated仍另验B5/B13 |
...
3. **先得到可解释的测量**：固定硬件、stream/file/filesystem映射、durability、TLS范围、E/W/A及负载，保留原写/读/重建/回收、index coverage、拒绝/retry/replacement、内存与四项磁盘指标。另分列compaction copied bytes和扣除真实destination占用后的net reusable bytes gained，关联debt及前台p99；满载暂停/恢复下游时记录已准入完成及资源归还。统一N、共享工作集、victim组/净收益阈值、维护/预分配上限均由同一manifest冻结；理想r/(1-r)不作为实测结果，未运行范围保持NOT_EXECUTED，不新建平台。
...
本轮归入既有UW-3/4/5/6/7与Spike A/B/C，CLIENT-1/2及UW-1/2保留，不重新编号。四组场景：未fenced副本先missing再接收旧写；入口满载暂停/恢复下游后已准入请求继续完成；低收益compaction按成组净收益背压/恢复；高live比例shared block在delete/部分move/orphan下仍按整单元条件FREE。优先闭合恢复否定证据和满载推进，compaction按简单策略测净收益，并清理重复Bookie/Arena职责。首批一Arena/固定shard/shared block、原E/W/A范围及CLIENT-1的4/3/2兼容回归不变；不另建基础quorum、证明服务、持久任务系统、全局sequencer或逐entry日志。
